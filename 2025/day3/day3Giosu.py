'''
--- Day 3: Lobby ---
You descend a short staircase, enter the surprisingly vast lobby, 
and are quickly cleared by the security checkpoint. 
When you get to the main elevators, however, 
...
"Some kind of electrical surge seems to have fried them. 
...
a value from 1 to 9. You make a note of their joltage ratings (your puzzle input). 
For example:
...
corresponds to a single bank of batteries. Within each bank, 
you need to turn on exactly two batteries; the joltage that the bank produces 
is equal to the number formed by the digits on the batteries you've turned on.
...
def part1(bank):
    ...
    for batteries in bank:
        best = 0
        nBatteries = len(batteries)

        # iter all possible combinations
        for i in range(nBatteries):
            for j in range(i + 1, nBatteries):
                
                #check if we have the newBest
                newBest = int(batteries[i])*10 + int(batteries[j])
                if(newBest > best):
                    best = newBest
